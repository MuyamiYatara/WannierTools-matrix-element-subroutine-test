  ...
  module para
     ...
     logical :: is_unpolarized_arpes ! if or not we are considering unpolarized light incoming
     ...
     namelist /PARAMETERS/ Eta_Arc, OmegaNum, OmegaNum_unfold, OmegaMin, OmegaMax, &
        ...
        photon_energy_arpes, polarization_xi_arpes, polarization_phi_arpes, test_namelist,&
        polarization_alpha_arpes, polarization_delta_arpes, is_unpolarized_arpes, penetration_lambda_arpes
...
module element_table
   ...
   polarization_delta_arpes, is_unpolarized_arpes, penetration_lambda_arpes
...
module me_calculate
   ...
   ! real(dp) :: epsilon_dot_kf_square = 1d0 ! polarization term result
    ...
    subroutine gaunt(l, m, dl, dm, gaunt_value)
        ...
        gaunt_value = 0d0
        write(*,*) 'we have wrong dm, dl value '
        
        return 
    end subroutine gaunt


    !> This subroutine is used to calculate the polarization term under the incoming light is linear polarized, not included in subroutine get_matrix_element !!!
    subroutine polarization_vector(polar_epsilon)
        implicit none
        complex(dp) :: polar_epsilon(3)
        write(*,*) polarization_phi_arpes
   ...
   !!!!end function Ylm
    ...
    subroutine get_matrix_element(ele_name, orb_name, k_cart, matrix_element_res)
        ...
        real(dp) :: gaunt_vec(3) ! values of gaunt coefficient corresponding to polarization vector
        ...
        do j = 0, 1
            ...
            call gaunt(l, m, dl, 1, gaunt_vec(1))
            call gaunt(l, m, dl, -1, gaunt_vec(2))
            call gaunt(l, m, dl, 0, gaunt_vec(3))

            !> calculate the matrix element
    
            
            matrix_element_res = matrix_element_res + 4d0*Pi*Ylm(k_theta, k_phi, l+dl, m)*inte_res*dot_product(polar_epsilon_vec, gaunt_vec)
        end do ! j loop
