  ...
  ! write(123,'(7(a12))')'#Column', '1', '2', '3', '4', '5', '6'
   ...
   if (Is_Sparse_Hr) then
      ...
      if (Landaulevel_unfold_line_calc.and..not.Is_Sparse_Hr) then
         ...
         neval= Num_wann*nslab
         nvecs= Num_wann*nslab
         allocate( hamk_bulk(num_wann*nslab, num_wann*nslab))
      endif
   endif
   allocate( W( neval))
   allocate( psi(Num_wann*nslab))
   allocate( zeigv(Num_wann*nslab,nvecs))
   ...
   do ik= 1+cpuid, nk3_band, num_cpu
      ...
      if (Landaulevel_unfold_line_calc) then 
         ...
         if (Is_Sparse_Hr) then
            ...
            call ham_slab(k_SBZ_direct(1:2), zeigv)
            ...
            call eigensystem_c('V', 'U', Num_wann*nslab ,zeigv, W)
         endif
      ...
      do n= 1, neval
         ...
         if (Landaulevel_unfold_line_calc) then
            ...
            call get_projection_weight_bulk_unfold(num_wann*nslab, k_SBZ_direct, k_PBZ_direct, psi, weight, Origin_cell, &
              NumberofSelectedOrbitals_groups_local, NumberofSelectedOrbitals_local, Selected_WannierOrbitals_local)
         endif
         do ig=1, NumberofSelectedOrbitals_groups_local
...
subroutine get_projection_weight_bulk_unfold(ndim, k_SBZ_direct, k_PBZ_direct, psi, weight, origincell, &
   ...
   !! calculate the matrix element values
   ...
   do ig=1, NumberofSelectedOrbitals_groups
      ...
      do io_PC=1, Folded_cell%NumberofSpinOrbitals
         ...
         do io=1, NumberofSelectedOrbitals(ig)
            ...
            !call get_matrix_element(atom_name_SC, projector_name_SC, k_cart, me) !move away
