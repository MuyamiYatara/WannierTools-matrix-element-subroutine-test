...
subroutine unfolding_kpath
   ...
   do ik= 1+cpuid, nk3_band, num_cpu
      !if (cpuid==0) 
      write(stdout, '(a, i10, i10," /", i10)') 'BulkBand unfolding at :',cpuid, ik, nk3_band
      ...
      do n= 1, neval
         ...
         do ig=1, NumberofSelectedOrbitals_groups_local
            ...
            enddo ! ieta
         enddo ! ig
         write(stdout, *) "n", n, W(n)
         !write(*, *) "n", n, W(n)/eV2Hartree
      enddo ! sum over n
   enddo !ik
   ...
   if (cpuid.eq.0)then
      ...
      do ik=1, nk3_band
         do ie=1, omeganum_unfold
            write(outfileindex, '(300f32.8)')k3len_unfold(ik), omega(ie)/eV2Hartree, &
               ((spectrum_unfold_mpi(ie, ieta, ig, ik), ieta=1, NumberofEta), ig=1, NumberofSelectedOrbitals_groups_local)
            !write(*,*) spectrum_unfold(ie, ieta, ig, ik)
         enddo
         write(outfileindex, *) ' '
      enddo
      close(outfileindex)
...
subroutine unfolding_kplane
   ...
   if (cpuid.eq.0)then
      ...
      do ik=1, knv3
         ik1= ik12(1, ik)
         ik2= ik12(2, ik)
         write(outfileindex, '(3000f32.5)')kxy_shape(:, ik)*Angstrom2atomic, kxy_plane(:, ik)*Angstrom2atomic, &
...
subroutine get_projection_weight_bulk_unfold(ndim, k_SBZ_direct, k_PBZ_direct, psi, weight, origincell, &
   ...
   !> local variables
   integer :: io, io_SC, io_PC, projector_SC, projector_PC, num_of_proj
   ...
   real(dp) :: kdotr, r_abs, k_abs         
   complex(dp) :: overlp, overlp_i
   character(10) :: atom_name_PC, atom_name_SC, projector_name_PC !! orbital name
   complex(dp) :: me !! matrix element
   complex(dp), allocatable :: me_values(:)
   ...
   !write(*,*)"k_PBZ", k_PBZ_direct, k_cart
   ...
   do io_PC=1, Folded_cell%NumberofSpinOrbitals
      ...
      num_of_proj = 0
            do i = 1, Folded_cell%spinorbital_to_atom_index(io_PC)
               if (i == Folded_cell%spinorbital_to_atom_index(io_PC)) then
                  do j = 1, Folded_cell%nprojs(Folded_cell%spinorbital_to_atom_index(io_PC))
                     if(Folded_cell%spinorbital_to_projector_index(io_PC) == (num_of_proj+j)) then
                        projector_name_PC= adjustl(trim(Folded_cell%proj_name(j, i)))
                     end if
                  end do
               else
                  num_of_proj = num_of_proj + Folded_cell%nprojs(i)
               end if
            end do
   ...
   do ig=1, NumberofSelectedOrbitals_groups
      ...
      do io_PC=1, Folded_cell%NumberofSpinOrbitals
         ...
         do io=1, NumberofSelectedOrbitals(ig)
            ...
            !! we need to know the orbital name to calculate the matrix element
            
            !write(*,*) projector_name_SC
            ...
            !call direct_cart_real_unfold(posi_direct_unfold+irvec(:,iR), posi_cart)
            ...
            !kdotr=dot_product(posi_direct_unfold, k_SBZ_direct)
            !!calculate the |r|,|k|
            !r_abs = sqrt(dot_product(posi_cart, posi_cart))
            !if(k_cart  (0d0,0d0,0d0)) write(*,*) 'PC', tau_j_tilde, 'SC',tau_i_tilde
            ...
            overlp_i= overlp_i+ delta(0.1d0, norm(dij_tilde_cart))*psi(io_SC)/delta(0.1d0, 0d0)*(cos(2*pi*(kdotr))-zi*sin(2*pi*(kdotr)))*me_values(io_PC)
            !&
      ...
      weight(ig)= weight(ig)+ abs(overlp_i)**2 
   enddo ! ig
   !weight= weight*origincell%CellVolume
   weight= weight*origincell%CellVolume
   return
